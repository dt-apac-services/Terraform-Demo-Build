...
resource "google_compute_instance" "vm_instance" {
  ...
  metadata_startup_script = <<-EOT
    ...
    wget -O dynatrace-oneagent-latest.sh "${var.dt_tenant_url}/api/v1/deployment/installer/agent/unix/default/latest?arch=x86&flavor=default" --header="Authorization: Api-Token ${var.dt_paas_token}";
    # curl -X GET "${var.dt_tenant_url}/api/v1/deployment/installer/agent/unix/default/latest?flavor=default&arch=all&bitness=all&skipMetadata=false&networkZone=default" -H "accept: */*" -H "Authorization: Api-Token ${var.dt_paas_token}" > /var/tmp/dynatrace-oneagent-latest.sh;
